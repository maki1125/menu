...
class MaterialRepository {
  ...
  factory MaterialRepository() {
    ...
    if (_instance!.user.uid != firebaseUser!.uid) {
      throw Exception(
          "MaterialRepository is already initialized with a different user.");
    }
  ...
  Stream<List<Map<String, dynamic>>> getMaterialList() {
    return FirebaseFirestore.instance
      ...
      for (final change in snapshot.docChanges) {
        final material = MaterialModel.fromFirestore(
            change.doc.data() as Map<String, dynamic>);

        switch (change.type) {
          //追加ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
          case DocumentChangeType.added:
            LoggerService.debug("add_material:${material.id}");
            if ((material.id == "noData" &&
                    !materialList.any((m) => ("noData" ==
                        material.id))) //データ追加された時にまだidがついていない場合がある。
                ||
                !materialList.any((m) => (m["id"] == material.id))) {
              //すでにリストにある場合は追加しない。初回に2回addしてしまうため。
  ...
  Future<void> deleteMaterial(MaterialModel material) async {
    db.collection('users/${user.uid}/materials').doc(material.id).delete();
  }
